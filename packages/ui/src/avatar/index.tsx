...
import { avatarGroupStyles, avatarStyles } from "./styles";
...
const _Avatar = forwardRef<HTMLSpanElement, PropTypes>(
  ({ rootProps, size, color, className, fallbackProps, ...props }, ref) => {
    return (
      <RadixAvatar.Root ref={ref} {...rootProps}>
        <RadixAvatar.Image {...props} className={avatarStyles({ size, className, color })} />
        {!props.src ? (
...
const Group = forwardRef<HTMLDivElement, GroupProps>(({ spacing, className, ...props }, ref) => {
  return (
    <div ref={ref} className={avatarGroupStyles({ spacing, className })}>
      {props.children}
    </div>
  );
});
